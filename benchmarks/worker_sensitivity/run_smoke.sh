#!/bin/bash
# Smoke della sensitivity: la stessa corsa, sei utenti soli e la traccia breve.
# $1=nome della corsa (default smoke) $2=worker attesi (default 2) $3=utenti per worker (default 3)
#
# L'override e' SEMPRE overrides/override_smoke.yaml, qualunque sia il nome
# della corsa: il nome batezza soltanto i file prodotti.
#
...
N="${1:-smoke}"; EW="${2:-2}"; EPW="${3:-3}"
OVERRIDE="$SCENARIO_DIR/overrides/override_smoke.yaml"
[ -f "$OVERRIDE" ] || { echo "override assente: $OVERRIDE" >&2; exit 1; }

echo "### SMOKE $N: $EW worker attesi, $EPW utenti ciascuno"
...
if [ ! -f "$SCENARIO_DIR/traces/smoke_trace.sha256" ]; then
  echo "FALLITA $N: manifest della traccia assente: $SCENARIO_DIR/traces/smoke_trace.sha256" >&2
  exit 6
fi
if ! ( cd "$SCENARIO_DIR/traces" && sha256sum -c smoke_trace.sha256 ); then
  echo "FALLITA $N: la traccia smoke_trace.jsonl non corrisponde al suo hash" >&2
  exit 6
fi
echo "  traccia smoke_trace.jsonl: hash verificato"
...
python3 "$SCENARIO_DIR/worker_probe.py" --config "$N" --base http://127.0.0.1:8098 \
  ...
  --journal "$LAB_DIR/runtime/orders.decisions.jsonl" --trace "$SCENARIO_DIR/traces/smoke_trace.jsonl" \
  --out "$WORK_DIR/$N" --expect-workers "$EW" --expect-per-worker "$EPW" --users 6
rc=$?
